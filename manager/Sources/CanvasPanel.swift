                    ...
                    if isHovered {
                        HStack(spacing: 4) {
                            if let copied = copiedText {
                                ...
                                RowButton(label: "Open") {
                                    manager.openBrowserOnly(item)
                                }
                                RowButton(label: "Copy") {
                                    showCopied("URL copied!")
                                    manager.copyURL(item)
                                }
                                RowButton(label: "Stop", destructive: true) {
                                    killWithAnimation()
                                }
                            } else {
                                RowButton(label: "Start") {
                                    manager.startCanvas(item)
                                }
                                if item.entry.mostRecentSessionId != nil {
                                    RowButton(label: "Resume") {
                                        showCopied("Copied!")
                                        manager.copyResumeCommand(item)
                                    }
                                }
                            }
                        }
